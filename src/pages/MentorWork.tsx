...
export default function MentorWork({ adminUserId, isTasdiqlovchi }: { adminUserId?: string, isTasdiqlovchi?: boolean }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (data?.table_data) {
        ...
        setIsEditing(false);
        localStorage.setItem('cache_mentor_work_' + targetUserId, JSON.stringify({ grid: loadedGrid, historyLogs: loadedLogs }));
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSavedData(grid);
      setIsEditing(false);
      localStorage.setItem('cache_mentor_work_' + targetUserId, JSON.stringify({ grid, historyLogs }));
      setSuccess(true);
