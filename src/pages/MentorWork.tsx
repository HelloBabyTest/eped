...
export default function MentorWork({ adminUserId, isTasdiqlovchi }: { adminUserId?: string, isTasdiqlovchi?: boolean }) {
  const { t } = useLanguage();

  const cachedData = (() => {
    const userId = adminUserId || localStorage.getItem('current_user_id') || '';
    if (!userId) return null;
    const cache = localStorage.getItem('cache_mentor_work_' + userId);
    if (cache) {
  ...
  useEffect(() => {
    const fetchAppr = async () => {
      ...
      if (uid) {
        setIsApproved(localStorage.getItem('approved_mentor_' + uid) === 'true');
      }
    };
    fetchAppr();
  }, [adminUserId]);
  ...
  const toggleApproval = () => {
      ...
      localStorage.setItem('approved_mentor_' + adminUserId, newVal ? 'true' : 'false');
  };
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (data?.table_data) {
        ...
        setIsEditing(false);
        localStorage.setItem('cache_mentor_work_' + targetUserId, JSON.stringify(tableData));
      } else {
        setGrid(DEFAULT_TEMPLATE);
        setSavedData(DEFAULT_TEMPLATE);
        setIsEditing(true);
        localStorage.setItem('cache_mentor_work_' + targetUserId, JSON.stringify(DEFAULT_TEMPLATE));
      }
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSavedData(grid);
      setIsEditing(false);
      localStorage.setItem('cache_mentor_work_' + targetUserId, JSON.stringify(grid));
      setSuccess(true);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-bold text-gray-900">{t('masterApprentice')}</h1>
            {isApproved && (
          ...
          <p className="text-gray-500 mt-1">Ustoz-shogird ishlarini boshqarish jadvali.</p>
        </div>
